...
class MPRViewer(QMainWindow):
    ...
    def load_data(self):
        ...
        folder_path = QFileDialog.getExistingDirectory(self, "Select Folder Containing DICOM or NIfTI files",
                                                       options=options)
        if folder_path:
    ...
    def apply_brightness_contrast(self, img, viewer_index):
        return np.clip(
            (img - img.min()) / (img.max() - img.min()) * self.contrast[viewer_index] + self.brightness[viewer_index],
            0, 1)
